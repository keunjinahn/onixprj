    ...
    <template v-slot>
      <div class="main-panel">
        ...
        <v-data-table
          ...
          <template v-slot:[`item.sensor_id`]="{item}">
            {{String(item.sensor_id).padStart(3,'0')}}
          </template>            
...
export default {
  ...
  methods: {
    ...
    openModifyPopup(item,type){
      ...
      this.addPopup.form.sensor_id = String(item.sensor_id).padStart(3,'0')
    ...
    async addSensor() {
      ...
      let param = {
        ...
        sensor_idx: String(this.addPopup.form.customer_idx).padStart(5,'0') + "_" + String(this.addPopup.form.receiver_id).padStart(3,'0') 
        + "_" + String(this.addPopup.form.system_id).padStart(3,'0') + "_" + String(this.addPopup.form.repeater_id).padStart(3,'0')
        + "_" + String(this.addPopup.form.sensor_id).padStart(3,'0'),
        ...
        sensor_id:this.addPopup.form.sensor_id,
      };
      if(this.addPopup.popup_type == 'ADD') {
        await this.$http.post("sensor", param)
      }else {
        await this.$http.patch(`sensor/${this.addPopup.form.id}`, param)
      }
