...
const args = C.parseUriArgs()

// Construct a MeshServer agent direction object
export default function CreateAgentRedirect(meshserver, module, serverPublicNamePort, authCookie, domainUrl) {
  var obj = {};
  ...
  obj.Start = function (nodeid) {
    ...
    var url2, url = `${process.env.VUE_APP_WS_ADDR}${domain}/meshrelay.ashx?browser=1&p=${obj.protocol}&nodeid=${nodeid}&id=${obj.tunnelid}`
    // url = window.location.protocol.replace("http", "ws") 
    //   + "//" + window.location.host 
    //   + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')) 
    //   + "/meshrelay.ashx?browser=1&p=" + obj.protocol + "&nodeid=" + nodeid + "&id=" + obj.tunnelid;
    ...
    obj.meshserver.send({
      ...
      type: 'tunnel',
      nodeid: obj.nodeid,
      value: "*" + domainUrl + "meshrelay.ashx?p=" + obj.protocol + "&nodeid=" + nodeid + "&id=" + obj.tunnelid,
      usage: obj.protocol
    });
  ...
  obj.xxOnControlCommand = function (msg) {
    var controlMsg;
    try {
      controlMsg = JSON.parse(msg);
    } catch (e) {
      return;
    }
    if (controlMsg.ctrlChannel != '102938') {
      obj.xxOnSocketData(msg);
      return;
    }
    //console.log(controlMsg);
    if (controlMsg.type == 'console') {
      obj.consoleMessage = controlMsg.msg;
      if (obj.onConsoleMessageChange) {
        obj.onConsoleMessageChange(obj, obj.consoleMessage);
      }
    } else if (obj.webrtc != null) {
      if (controlMsg.type == 'answer') {
        obj.webrtc.setRemoteDescription(new RTCSessionDescription(controlMsg), function () {
          /*console.log('WebRTC remote ok');*/
        }, obj.xxCloseWebRTC);
  ...
  obj.sendCtrlMsg = function (x) {
    if (obj.ctrlMsgAllowed == true) {
      if ((typeof args != 'undefined') && args.redirtrace) {
        console.log('RedirSend', typeof x, x);
      }
      try {
        obj.socket.send(x);
      } catch (ex) {}
  ...
  function performWebRtcSwitch() {
    if ((obj.webSwitchOk == true) && (obj.webRtcActive == true)) {
      ...
      if (obj.onStateChanged != null) {
        obj.onStateChanged(obj, obj.State);
      }
    }
  }
  ...
  obj.xxOnMessage = function (e) {
    ...
    if (obj.State < 3) {
      if ((e.data == 'c') || (e.data == 'cr')) {
        ...
        if (obj.attemptWebRTC == true) {
          ...
          //   obj.webrtc = new RTCPeerConnection(configuration);
          // } else if (typeof webkitRTCPeerConnection !== 'undefined') {
          //   obj.webrtc = new webkitRTCPeerConnection(configuration);
          // }
          ...
          if (obj.webrtc != null) {
            ...
            obj.webchannel.onclose = function (event) {
              if (obj.webRtcActive) {
                obj.Stop();
              }
            }
            obj.webrtc.onicecandidate = function (e) {
              if (e.candidate == null) {
                try {
                  obj.socket.send(JSON.stringify(obj.webrtcoffer));
                } catch (ex) {} // End of candidates, send the offer
              } else {
    ...
    */
    ...
    if (typeof e.data == 'object') {
      if (fileReaderInuse == true) {
        fileReaderAcc.push(e.data);
        return;
      }
  ...
  var fileReaderInuse = false,
    fileReaderAcc = [];
  if (fileReader.readAsBinaryString) {
    // Chrome & Firefox (Draft)
    fileReader.onload = function (e) {
      obj.xxOnSocketData(e.target.result);
      if (fileReaderAcc.length == 0) {
        fileReaderInuse = false;
      } else {
        fileReader.readAsBinaryString(new Blob([fileReaderAcc.shift()]));
      }
    }
  } else if (fileReader.readAsArrayBuffer) {
    // Chrome & Firefox (Spec)
    fileReader.onloadend = function (e) {
      ...
      if (fileReaderAcc.length == 0) {
        ...
        fileReader.readAsArrayBuffer(fileReaderAcc.shift());
      }
    }
  }
  ...
  obj.xxOnSocketData = function (data) {
    ...
    if ((typeof args != 'undefined') && args.redirtrace) {
      console.log("RedirRecv", typeof data, data.length, data);
    }
  ...
  obj.sendText = function (x) {
    if (typeof x != 'string') {
      x = JSON.stringify(x);
    } // Turn into a string if needed
    obj.send(C.encode_utf8(x)); // Encode UTF8 correctly
  }
  ...
  obj.send = function (x) {
    ...
    if ((typeof args != 'undefined') && args.redirtrace) {
      console.log('RedirSend', typeof x, x.length, x);
    }
    try {
      if (obj.socket != null && obj.socket.readyState == WebSocket.OPEN) {
        if (typeof x == 'string') {
          if (obj.debugmode == 1) {
            let b = new Uint8Array(x.length),
              c = [];
            for (let i = 0; i < x.length; ++i) {
              b[i] = x.charCodeAt(i);
              c.push(x.charCodeAt(i));
            }
            if (obj.webRtcActive == true) {
              obj.webchannel.send(b.buffer);
            } else {
  ...
  obj.xxCloseWebRTC = function () {
    if (obj.webchannel != null) {
      try {
        obj.webchannel.close();
      } catch (e) {}
      obj.webchannel = null;
    }
    if (obj.webrtc != null) {
    ...
    if (obj.socket != null) {
      try {
        if (obj.socket.readyState == 1) {
          obj.sendCtrlMsg("{\"ctrlChannel\":\"102938\",\"type\":\"close\"}");
          obj.socket.close();
        }
      } catch (e) {}
      obj.socket = null;
