...
export default function CreateAmtRedirect(module, authCookie) {
  var obj = {};
  ...
  obj.xxOnSocketData = function (data) {
    ...
    if ((obj.protocol == 2 || obj.protocol == 3) && obj.connectstate == 1) {
      ...
      switch (obj.amtaccumulator.charCodeAt(0)) {
        ...
        case 0x14: // AuthenticateSessionReply (20)
          ...
          for (i = 0; i < authDataLen; i++) {
            authData.push(obj.amtaccumulator.charCodeAt(9 + i));
          }
        ...
        case 0x21: // Response to settings (33)
          ...
          if (obj.protocol == 1) {
            obj.amtkeepalivetimer = setInterval(obj.xxSendAmtKeepAlive, 2000);
          }
          obj.connectstate = 1;
          obj.xxStateChange(3);
          break;
        ...
        case 0x41:
          ...
          if (obj.amtaccumulator.length > 8) {
            obj.m.ProcessData(obj.amtaccumulator.substring(8));
          }
  ...
  obj.send = function (x) {
    ...
    if (obj.protocol == 1) {
      obj.xxSend(String.fromCharCode(0x28, 0x00, 0x00, 0x00) + IntToStrX(obj.amtsequence++) + ShortToStrX(x.length) + x);
    } else {
      obj.xxSend(x);
    }
  }
  ...
  obj.xxRandomNonce = function (length) {
    var r = "";
    for (var i = 0; i < length; i++) {
      r += obj.xxRandomNonceX.charAt(Math.floor(Math.random() * obj.xxRandomNonceX.length));
    }
    return r;
  }
  ...
  obj.Stop = function (x) {
    ...
    if (obj.amtkeepalivetimer != null) {
      clearInterval(obj.amtkeepalivetimer);
      obj.amtkeepalivetimer = null;
    }
  }
