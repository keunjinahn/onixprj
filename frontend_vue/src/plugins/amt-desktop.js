...
export default function CreateAmtRemoteDesktop(divid, scrolldiv) {
  var obj = {};
  obj.canvasid = divid;
  obj.CanvasId = C.Q(divid);
  obj.scrolldiv = scrolldiv;
  obj.canvas = C.Q(divid).getContext("2d");
  obj.protocol = 2; // KVM
  obj.state = 0;
  obj.acc = "";
  obj.ScreenWidth = 960;
  ...
  obj.xxStateChange = function (newstate) {
    if (newstate == 0) {
      ...
      C.QS(obj.canvasid).cursor = 'default';
    } else {
      C.QS(obj.canvasid).cursor = obj.showmouse ? 'default' : 'none';
    }
  }
  ...
  obj.ProcessData = function (data) {
    ...
    while (obj.acc.length > 0) {
      ...
      if (obj.state == 0 && obj.acc.length >= 12) {
        ...
        // ###END###{DesktopRotation}
        var namelen = C.ReadInt(obj.acc, 20);
        if (obj.acc.length < 24 + namelen) return;
        cmdsize = 24 + namelen;
        obj.canvas.canvas.width = obj.rwidth = obj.width = obj.ScreenWidth = C.ReadShort(obj.acc, 0);
        obj.canvas.canvas.height = obj.rheight = obj.height = obj.ScreenHeight = C.ReadShort(obj.acc, 2);
        ...
        */
        ...
        if (obj.useZRLE) supportedEncodings += C.IntToStr(16);
        supportedEncodings += C.IntToStr(0);
        // ###BEGIN###{DesktopInband}
        supportedEncodings += C.IntToStr(1092);
        // ###END###{DesktopInband}

        obj.send(String.fromCharCode(2, 0) + C.ShortToStr((supportedEncodings.length / 4) + 1) + supportedEncodings + C.IntToStr(-223)); // Supported Encodings + Desktop Size
        ...
        if (obj.bpp == 1) obj.send(String.fromCharCode(0, 0, 0, 0, 8, 8, 0, 1) + C.ShortToStr(7) + C.ShortToStr(7) + C.ShortToStr(3) + String.fromCharCode(5, 2, 0, 0, 0, 0)); // Setup 8 bit color RGB332
        ...
        if (obj.onScreenSizeChange != null) {
          obj.onScreenSizeChange(obj, obj.ScreenWidth, obj.ScreenHeight);
        }
      } else if (obj.state == 4) {
        switch (obj.acc.charCodeAt(0)) {
          case 0: // FramebufferUpdate
            if (obj.acc.length < 4) return;
            obj.state = 100 + C.ReadShort(obj.acc, 2); // Read the number of tiles that are going to be sent, add 100 and use that as our protocol state.
            cmdsize = 4;
            break;
          ...
          case 3: // This is ServerCutText
            ...
            var len = C.ReadInt(obj.acc, 4) + 8;
            if (obj.acc.length < len) return;
            cmdsize = handleServerCutText(obj.acc);
            break;
        }
      } else if (obj.state > 100 && obj.acc.length >= 12) {
        var x = C.ReadShort(obj.acc, 0),
          y = C.ReadShort(obj.acc, 2),
          width = C.ReadShort(obj.acc, 4),
          height = C.ReadShort(obj.acc, 6),
          s = width * height,
          encoding = C.ReadInt(obj.acc, 8);

        if (encoding < 17) {
          if (width < 1 || width > 64 || height < 1 || height > 64) {
            console.log("Invalid tile size (" + width + "," + height + "), disconnecting.");
            return obj.Stop();
          }
          ...
          if (obj.sparew != width || obj.spareh != height) {
            ...
            if (obj.rotation == 1 || obj.rotation == 3) {
              obj.sparew2 = height, obj.spareh2 = width;
            }
            // ###END###{DesktopRotation}
        ...
        if (encoding == 0xFFFFFF21) {
          ...
          obj.send(String.fromCharCode(3, 0, 0, 0, 0, 0) + C.ShortToStr(obj.width) + C.ShortToStr(obj.height)); // FramebufferUpdateRequest
          cmdsize = 12;
          if (obj.onScreenSizeChange != null) {
            obj.onScreenSizeChange(obj, obj.ScreenWidth, obj.ScreenHeight);
          }
          ...
          if (obj.bpp == 2) {
            for (let i = 0; i < s; i++) {
              _setPixel16(obj.acc.charCodeAt(ptr++) + (obj.acc.charCodeAt(ptr++) << 8), i);
            }
          } else {
            for (let i = 0; i < s; i++) {
              _setPixel8(obj.acc.charCodeAt(ptr++), i);
            }
          }
          ...
          var datalen = C.ReadInt(obj.acc, 12);
          ...
          if (datalen > 5 && obj.acc.charCodeAt(ptr) == 0 && C.ReadShortX(obj.acc, ptr + 1) == (datalen - delta)) {
          ...
          // ###BEGIN###{Inflate}
            ...
            if (arr.length > 0) {
              _decodeLRE(arr, 0, x, y, width, height, s, arr.length);
            } else {
              obj.Debug("Invalid deflate data");
            }
          }
          // ###END###{Inflate}
  ...
  function _decodeLRE(data, ptr, x, y, width, height, s, datalen) {
    var subencoding = data.charCodeAt(ptr++),
      index, v, runlengthdecode, palette = {},
      rlecount = 0,
      runlength = 0,
      i;
    ...
    if (subencoding == 0) {
      ...
      if (obj.bpp == 2) {
        for (i = 0; i < s; i++) {
          _setPixel16(data.charCodeAt(ptr++) + (data.charCodeAt(ptr++) << 8), i);
        }
      } else {
        for (i = 0; i < s; i++) {
          _setPixel8(data.charCodeAt(ptr++), i);
        }
      }
      _putImage(obj.spare, x, y);
      ...
      // Read the palette
      var br = 4,
        bm = 15; // br is BitRead and bm is BitMask. By adjusting these two we can support all the variations in this encoding.
      if (obj.bpp == 2) {
        for (i = 0; i < subencoding; i++) {
          palette[i] = data.charCodeAt(ptr++) + (data.charCodeAt(ptr++) << 8);
        }
        if (subencoding == 2) {
          br = 1;
          bm = 1;
        ...
        while (rlecount < s && ptr < data.length) {
          ...
          for (i = (8 - br); i >= 0; i -= br) {
            _setPixel16(palette[(v >> i) & bm], rlecount++);
          }
        } // Display all the bits
      } else {
        for (i = 0; i < subencoding; i++) {
          palette[i] = data.charCodeAt(ptr++);
        }
        if (subencoding == 2) {
          br = 1;
          bm = 1;
        ...
        while (rlecount < s && ptr < data.length) {
          v = data.charCodeAt(ptr++);
          for (i = (8 - br); i >= 0; i -= br) {
            _setPixel8(palette[(v >> i) & bm], rlecount++);
          }
        } // Display all the bits
      }
      _putImage(obj.spare, x, y);
      ...
      if (obj.bpp == 2) {
        while (rlecount < s && ptr < data.length) {
          ...
          // Draw a run
          if (obj.rotation == 0) {
            _setPixel16run(v, rlecount, runlength);
            rlecount += runlength;
          } else {
            while (--runlength >= 0) {
              _setPixel16(v, rlecount++);
            }
          }
        }
      } else {
        while (rlecount < s && ptr < data.length) {
          ...
          // Draw a run
          if (obj.rotation == 0) {
            _setPixel8run(v, rlecount, runlength);
            rlecount += runlength;
          } else {
            while (--runlength >= 0) {
              _setPixel8(v, rlecount++);
            }
          }
        }
      }
  ...
  obj.hold = function (holding) {
    ...
    if (obj.holding == false) {
      ...
      if ((obj.canvas.canvas.width != obj.width) || (obj.canvas.canvas.height != obj.height)) {
        obj.canvas.canvas.width = obj.width;
        obj.canvas.canvas.height = obj.height;
        if (obj.onScreenSizeChange != null) {
          obj.onScreenSizeChange(obj, obj.ScreenWidth, obj.ScreenHeight);
        } // ???
      }
  ...
  function _setPixel8(v, p) {
    ...
    if (obj.rotation > 0) {
      if (obj.rotation == 1) {
        let x = p % obj.sparew,
          y = Math.floor(p / obj.sparew);
        p = (x * obj.sparew2) + (obj.sparew2 - 1 - y);
        pp = p << 2;
      } else if (obj.rotation == 2) {
        pp = (obj.sparew * obj.spareh * 4) - 4 - pp;
  ...
  function _setPixel8run(v, p, run) {
    ...
    while (--run >= 0) {
      obj.spare.data[pp] = r;
      obj.spare.data[pp + 1] = g;
      obj.spare.data[pp + 2] = b;
      pp += 4;
    }
  }
  ...
  function _SendRefresh() {
    ...
    if (obj.focusmode > 0) {
      ...
      obj.send(String.fromCharCode(3, 1) + C.ShortToStr(Math.max(Math.min(obj.ox, obj.mx) - obj.focusmode, 0)) + C.ShortToStr(Math.max(Math.min(obj.oy, obj.my) - obj.focusmode, 0)) + C.ShortToStr(df + Math.abs(obj.ox - obj.mx)) + C.ShortToStr(df + Math.abs(obj.oy - obj.my))); // FramebufferUpdateRequest
      obj.ox = obj.mx;
      obj.oy = obj.my;
    } else {
      // ###END###{DesktopFocus}
      // Request the entire screen
      obj.send(String.fromCharCode(3, 1, 0, 0, 0, 0) + C.ShortToStr(obj.rwidth) + C.ShortToStr(obj.rheight)); // FramebufferUpdateRequest
      // ###BEGIN###{DesktopFocus}
    }
    // ###END###{DesktopFocus}
  }
  ...
  obj.Start = function () {
    ...
    obj.kvmDataSupported = false;
    // ###END###{DesktopInband}
    for (var i in obj.sparecache) {
      delete obj.sparecache[i];
    }
  }
  ...
  */
  ...
  obj.sendkey = function (k, d) {
    if (typeof k == 'object') {
      ...
      var buf = '';
      for (var i in k) {
        buf += (String.fromCharCode(4, k[i][1], 0, 0) + C.IntToStr(k[i][0]));
      }
      obj.send(buf);
    } else {
      obj.send(String.fromCharCode(4, d, 0, 0) + C.IntToStr(k));
    }
  }
  ...
  function handleServerCutText(acc) {
    ...
    if (obj.onKvmData != null) {
      ...
      if ((d.length >= 16) && (d.substring(0, 15) == '\0KvmDataChannel')) {
        if (obj.kvmDataSupported == false) {
          obj.kvmDataSupported = true;
          console.log('KVM Data Channel Supported.');
        }
        if (((obj.onKvmDataAck == -1) && (d.length == 16)) || (d.charCodeAt(15) != 0)) {
          obj.onKvmDataAck = true;
        }
        //if (urlvars && urlvars['kvmdatatrace']) { console.log('KVM-Recv(' + (d.length - 16) + '): ' + d.substring(16)); }
        if (d.length >= 16) {
          obj.onKvmData(d.substring(16));
        } // Event the data and ack
        if ((obj.onKvmDataAck == true) && (obj.onKvmDataPending.length > 0)) {
          obj.sendKvmData(obj.onKvmDataPending.shift());
        } // Send pending data
      }
    }
    // ###END###{DesktopInband}
  ...
  obj.sendKvmData = function (x) {
    if (obj.onKvmDataAck !== true) {
      ...
      obj.send(String.fromCharCode(6, 0, 0, 0) + C.IntToStr(x.length) + x);
  ...
  obj.sendKeepAlive = function () {
    if (obj.lastKeepAlive < Date.now() - 5000) {
      obj.lastKeepAlive = Date.now();
      obj.send(String.fromCharCode(6, 0, 0, 0) + C.IntToStr(16) + '\0KvmDataChannel\0');
    }
  }
  // ###END###{DesktopInband}
  ...
  } // Control down, Alt down, Delete down, Delete up , Alt up , Control up
  ...
  obj.handleKeyUp = function (e) {
    return _keyevent(0, e);
  }
  obj.handleKeyDown = function (e) {
    return _keyevent(1, e);
  }
  ...
  obj.mousedown = function (e) {
    obj.buttonmask |= (1 << e.button);
    return obj.mousemove(e, 1);
  }
  obj.mouseup = function (e) {
    obj.buttonmask &= (0xFFFF - (1 << e.button));
    return obj.mousemove(e, 1);
  }
  obj.mousemove = function (e, force) {
    if (obj.state < 4) return true;
    var ScaleFactorHeight = (obj.canvas.canvas.height / C.Q(obj.canvasid).offsetHeight);
    var ScaleFactorWidth = (obj.canvas.canvas.width / C.Q(obj.canvasid).offsetWidth);
    var Offsets = obj.getPositionOfControl(C.Q(obj.canvasid));
    ...
    if (force == 1) {
      obj.send(String.fromCharCode(5, obj.buttonmask) + C.ShortToStr(obj.mx) + C.ShortToStr(obj.my));
      if (obj.mNagleTimer != null) {
        clearTimeout(obj.mNagleTimer);
    ...
    //     q = C.QS('DeskFocus'),
    //     ppos = obj.getPositionOfControl(C.Q(obj.canvasid).parentElement);
